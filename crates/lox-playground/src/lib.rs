...
async fn playground(port: u16) {
    ...
    const FAVICON_PNG: &[u8] = include_bytes!("../res/favicon.png");
    ...
    let favicon_png = warp::path("favicon.png").map(|| {
        http::response::Builder::new().header("Content-Type", "image/png").body(FAVICON_PNG)
    });
    ...
    // Serve routes.
    let routes =
        warp::get().and(index_html.or(favicon_png).or(lox_js).or(lox_bg_wasm).or(worker_js));
