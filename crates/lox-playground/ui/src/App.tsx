...
function Nav(props: NavProps): JSX.Element {
  let runIcon = "bi-play-fill";
  let runColor = "btn-success";
  let runText = "Run";
  if (props.isRunning) {
    runIcon = "bi-stop-fill";
    runColor = "btn-danger";
    runText = "Stop";
  }

  return (
    <nav className="navbar navbar-light border bg-light px-3" id="navbar">
      ...
      <a
        id="run-btn"
        className={`btn bi ${runIcon} ${runColor}`}
        onClick={props.runClick}
      >
        {runText}
      </a>
    </nav>
...
function Output(props: OutputProps): JSX.Element {
  return (
    <pre
      className="h-100 bg-light font-monospace fs-6"
